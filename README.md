...
首个接入的上游是火山方舟 Seedance；同类上游的接入目标是「**4 行配置 + 渠道持证**」。
...
| 结果 | `GET /results/{task_id}` → 302 预签名 URL；转存永久失败 → 410 |
...
make test           # 184 项测试（SQLite + 内存 broker + MockTransport 假上游，无需 Redis）
