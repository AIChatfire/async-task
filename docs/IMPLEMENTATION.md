...
重投**不再**受乘子影响；乘子本身也会自己恢复（第 3 件），但恢复是**逐步**的（每成功调用 -10%，
且要过静默期），需要"立即复位"时用 §3.19 的治理面接口，不必再手工删 Redis 键。
...
### 3.19 渠道自适应（限流）状态：可观测 + 受支持的复位（2026-09-21，v0.0.6）

依据：livetest-ai 最新报告 `20260921T203719-…405b6a`（v0.0.5，27/27 通过，**T21 把 §3.18 的 F5
修复验完**）里**仍然挂着**的两条，以及 F5 自己写下的 impact：

1. **配置告警③**：「上游 id、持久层与 **poll 自适应状态**必须同轮归零 …… `poll:aimd/pause/hist`
   （Redis 卷、TTL 7 天）**跨部署保留**，历轮 429 剧本可把乘子推到上限 20×（不清则新轮全部轮询/
   重投都变 60s 节奏）」——测试侧的做法是**手工删 Redis 键**（"reset 段已改为清零"）。
   按纪律：**测试侧的 workaround = 产品侧缺一条正经路径**；
2. **`203053` 轮的 T21 抖动**：`乘子前=18.0`（用例期望 ≥20 的上限态）—— 上一轮留下的 20× 在静默期
   满后被一次**成功调用**衰减了一步（20×0.9=18），而用例把"污染到上限"当作可复现前提。
   根因是**状态不可控**：构造它要"赌"时序，读取它要翻 Redis；
3. **F5 的运维缺口**：报告 impact 写「**限流恢复后不会自动复活**」。乘子确实会衰减（§3.18 第 3 件），
   但那是**逐步**的（每成功调用 -10%，且要先过 `AG_AIMD_QUIET_SECONDS`）；运维手上没有"立即复位"的杆。

落地两件（都在治理面，**不新增配置项**）：

1. **查看** `GET /admin/channels/{channel}/pacing` —— 乘子 / 暂停位 / 上次 429 时刻 / 样本数 /
   乘子上限，外加**两个间隔并排给出**：`next_poll_interval`（含乘子 = 轮询节奏）与
   `resubmit_base_interval`（不含乘子 = 重投基准）。把 §3.18 的两条曲线做成"看得见"的事实，
   运维不必去 Redis 里翻键；F5 那句"重投不再被乘子放大"从此可以被**现场核对**。
2. **复位** `POST /admin/channels/{channel}/pacing/reset` —— 删 `poll:aimd` / `poll:pause` / `poll:rl`
   ⇒ 乘子回 `AIMD_MIN`、暂停位清零、429 时刻清空。`{"include_samples": true}` 才连 `poll:hist` /
   `poll:pct` 一起清（**默认不清**：限流惩罚与时长学习是两件事，复位前者不必牺牲后者）。
   写操作要 `operator`/`approver`，append-only 审计记下 `before`/`after` 乘子与是否清样本；
   **刻意不做双人复核**：它只影响"节奏"，不改任何任务状态、不触上游调用（与重放/灰度那类高危动作
   不同），双人复核在这里只是流程税。

实现落点：`infra/polling.py` 的 `current_multiplier` / `snapshot` / `reset`（Protocol + Redis 版 +
内存版**三处同语义**；方法名不叫 `multiplier` 是因为内存版把该名字用作"渠道 → 乘子"的字典，
方法与属性同名会互相遮蔽）、`admin/app.py` 两个端点。**渠道名要进 Redis 键**
（`poll:aimd:{channel}` 等）⇒ 按 `^[a-z0-9][a-z0-9._-]{0,63}$` 白名单收口：否则治理面就成了
"任意 Redis 键的读写把手"，`{…}` 这种 hash-tag 还能搅动集群分片。

守卫测试：`tests/test_admin.py`（5 条：鉴权/RBAC、两间隔语义、复位+审计+幂等、默认保留样本、
非法渠道名）、`tests/test_infra.py` 的 **Redis 真机**用例（键名/删键/样本保留 —— 内存实现证明不了
键型与键名，这正是 §3.10/F1 的教训）、`tests/test_rate_limit_deadline.py` 新增**顺延触顶跨周期饱和**
（补上报告"覆盖边界"里列为未覆盖的「顺延上限触顶」）。

**给下一轮的验证点**：① 测试侧的 reset 段可以改成**调接口**而不是直接删 Redis 键
（`POST /admin/channels/echo/pacing/reset`，必要时 `{"include_samples": true}`）；
② T21 的乘子前提改成**先复位再污染**，不再受上一轮残留影响（消除 `203053` 轮那种 18.0 抖动）；
③ 运维演练：限流恢复后复位一次，`next_poll_interval` 应立即等于 `resubmit_base_interval`。

**未验证**：本机无 Docker ⇒ 两个端点只在 in-process ASGI + 真 Redis 上验过（记录见提交正文）；
K8s 形态下的鉴权头注入（SSO）仍走契约。
